          ...
          <View style={styles.modalOverlay}>
            <View style={styles.modalContent}>
              <View style={styles.modalHeader}>
                ...
                <Text style={styles.modalTitle}>{modalMode === 'options' ? 'Import Itinerary' : 'Confirm Details'}</Text>
                <View style={{ width: 28 }} /> 
              </View>

              {modalMode === 'options' && (
                <>
                  <Text style={styles.subText}>Choose how you want to import the guest's flight data.</Text>
                  <TouchableOpacity style={styles.actionButton} onPress={handlePasteTicket}>
                    <View style={styles.iconCircle}><Ionicons name="clipboard-outline" size={24} color="#3B82F6" /></View>
                ...
                <View>
                  ...
                  <ScrollView horizontal showsHorizontalScrollIndicator={false} style={styles.propertyScroll}>
                    {properties.map(prop => (
                      ...
                      >
                        <Text style={[styles.propertyPillText, selectedPropertyId === prop.id && styles.propertyPillTextActive]}>{prop.name}</Text>
                      </TouchableOpacity>
                    ))}
                  </ScrollView>
...
const styles = StyleSheet.create({
  ...
  actionButton: { flexDirection: 'row', alignItems: 'center', backgroundColor: '#F8FAFC', borderWidth: 1, borderColor: '#E2E8F0', borderRadius: 16, padding: 16, marginBottom: 12 },
  ...
  input: { backgroundColor: '#F8FAFC', borderWidth: 1, borderColor: '#E2E8F0', borderRadius: 8, padding: 12, fontSize: 16, color: '#0F172A', marginBottom: 20 },
  rowInputs: { flexDirection: 'row', justifyContent: 'space-between' },
  propertyScroll: { marginBottom: 20, maxHeight: 50 },
  propertyPill: { paddingHorizontal: 16, paddingVertical: 10, borderRadius: 20, backgroundColor: '#F1F5F9', borderWidth: 1, borderColor: '#E2E8F0', marginRight: 8, alignSelf: 'flex-start' },
